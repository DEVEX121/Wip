	...
	if not isfile("vape/"..scripturl) then
		...
		if not suc or res == "404: Not Found" then
			if identifyexecutor and ({identifyexecutor()})[1] == 'Wave' then 
				displayErrorPopup('Stop using detected garbage, Vape will not work on such garabge until they fix BOTH HttpGet & file functions.Use solara instead')
				error(res)
			end
			displayErrorPopup("Failed to connect to gayhub : vape/"..scripturl.." : "..res)
			error(res)
		end
...
if not shared.VapeDeveloper then 
	...
	if commit then
		...
		displayErrorPopup("Failed to connect to github, please try using a VPN. DEVEX ON TOP")
		error("Failed to connect to github, please try using a VPN.DEVEX ON TOP")
	end
end
